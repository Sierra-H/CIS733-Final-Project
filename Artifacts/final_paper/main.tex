...
Our sources, recommended by Dr. Shamir, contained both phishing and legitimate emails. Our data came from three sources, the Enron dataset of corporate emails and the Ling dataset of emails were both sources of legitimate emails. Together, these datasets contained over five-hundred thousand emails. The third dataset, a set of emails generated by artificial intelligence and provided by Dr. Shamir, contained phishing emails. This data set contained 865 phishing emails.
...
This meant that the final dataset contained 2000 legitimate emails, and 865 phishing emails.

Once the data was sampled, the data then needed to be vectorized using udat. This vectorization then needed to be cleaned in order to remove NaN values that are disallowed in feature selection and outlier detection tools. These NaN values were replaced with the value 0. When required, the data was standardized for analysis. The following is the distribution of our data:

\includegraphics[scale=0.2]{data_1}
